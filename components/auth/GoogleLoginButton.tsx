  ...
  return (
    <div className="w-full">
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="w-full min-h-[52px] flex items-center justify-center gap-3 rounded-2xl bg-card border border-gray-200 text-ink text-lg font-bold shadow-sm active:scale-[0.98] transition disabled:opacity-60"
      >
        <GoogleIcon />
        {loading ? "이동 중..." : label}
      </button>
      {unavailable && (
        <p className="mt-2 text-sm text-danger text-center">
          지금은 Google 로그인을 사용할 수 없어요. 로그인 없이 시작해 주세요.
        </p>
      )}
    </div>
  );
